...
public class Community {
	...
	@Inline public def neigh_comm( node:int, start:int, end:int )
	{
	...
	public def one_level(  )
	{
		...
		{ //Console.OUT.println("nb_pass_done = " + nb_pass_done);
			...
			finish
			{
				...
				{
					val threadId:int=i;
					val start:int=threadId*band;
					val end:int=(threadId+1)*band;
					if (threadId<3) {
						async one_pass(start, end);	
					}
					else async one_pass(start, size);
				}
			}
	...
	public def one_pass(start:int, end:int)
	{
		var node:int;
		var node_comm:int;
		var w_degree:double;
		var neigh_weight:Array[double] = new Array[double](size+1,-1.0);
		var neigh_pos:Array[int] = new Array[int](size);
		...
		for (var node_tmp:int=start; node_tmp<=end;  node_tmp++) {
		/*if (level==1) 
		{
