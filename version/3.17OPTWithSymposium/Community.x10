...
public class Community {
	...
	public def modularity()
	{
		...
		for (i=0; i <= size; i++) {
			...
			if( total > 0 )
			{
				val out = total - inside;
				q += inside/m2 - (total/m2)*(total/m2);
			}	
		}
		return q;
	}
		...
		{ //Console.OUT.println("nb_pass_done = " + nb_pass_done);
			...
			for (var node_tmp:int=0; node_tmp<=size;  node_tmp++) {
				...
				remove(node, node_comm, neigh_weight(node_comm));
 				...
 				for ( i=0; i<neigh_last; i++) {
					val neigh_comm = neigh_pos(i);
					val neigh_weight = neigh_weight(neigh_comm);
 					increase = modularity_gain(node, neigh_comm, neigh_weight, w_degree);
 					computeTimes++;
 					if ((increase > best_increase)) {
 						best_comm = neigh_comm;
 						best_nblinks = neigh_weight;
